...
from model.ola_encoder import OLA_Classifier
...
def get_arguments():
    ...
    parser.add_argument('--ame_CMI', default=0.1, type=float, help='CMI parameter for AME module')
    parser.add_argument('--warmup_epoch', default=0, type=int, help='Number of warmup epochs before applying AME')
    parser.add_argument('--Use_MACE',default=False, type=bool, help="是否使用MACE loss")
    parser.add_argument('--random_mask_drop_prob',default=0.5, type=float, help="随机掩码的比例")
    ...
    parser.add_argument('--ame_use_adaptive_beta',default=True, type=bool, help="是否使用CMI")
    ...
    parser.add_argument("--Use_OLA", type=bool, default=False, help="是否使用OLA作为encoder")
    # 保存模型的相关参数
...
def train_epoch(args, epoch, model, device, dataloader, optimizer_m1, optimizer_m2, optimizer_fusion, optimizer_m3=None,scheduler_map=None):
    ...
    # For save setting 
    modal_loss_counts = [0 for _ in range(num_modalities)]
    ...
    for step, data_packet in enumerate(pbar):
        ...
        if modal_names == ["Visual", "Audio"]:
            ...
            # data_mini_packet = data_packet
            label = data_packet[-1].to(device)
            sid = data_packet[-2]
            data_mini_packet = data_packet[:-1]
            # outputs = model(data_mini_packet, epoch=epoch, labels=label, epoch_index=step)
        ...
        for idx, logits in enumerate(modal_logits):
            ...
            if idx < len(modal_loss_values):
                ...
                modal_loss_counts[idx] += 1

        loss_fusion = criterion(fusion_logits, label).mean()
        fusion_loss_sum += loss_fusion.detach().item()
        fusion_loss_count += 1
        ...
        # print(f"modal_acc_lists is {modal_acc_lists}")
    ...
    avg_loss_fusion = fusion_loss_sum / fusion_loss_count if fusion_loss_count else 0.0
    ...
    for idx, (name, acc_list, f1_list) in enumerate(zip(modal_names, modal_acc_lists, modal_f1_lists)):
        avg_acc = sum(acc_list) / len(acc_list) if acc_list else 0.0
        avg_f1 = sum(f1_list) / len(f1_list) if f1_list else 0.0
        loss_count = modal_loss_counts[idx]
        avg_loss = modal_loss_sums[idx] / loss_count if loss_count else 0.0
        modal_avg_metrics[name] = (avg_acc, avg_f1, avg_loss)

    
    print(f"\nTrain Epoch {epoch} Summary (Average):")
    print(f"  Fusion -> Accuracy: {avg_acc_fusion:.4f}, F1-Score: {avg_f1_fusion:.4f}, Loss: {avg_loss_fusion:.4f}")
    for name in modal_names:
        acc, f1, loss_val = modal_avg_metrics.get(name, (0.0, 0.0, 0.0))
        print(f"  {name} -> Accuracy: {acc:.4f}, F1-Score: {f1:.4f}, Loss: {loss_val:.4f}")

    return (avg_acc_fusion, avg_f1_fusion, avg_loss_fusion), modal_avg_metrics, MaskNumber
        ...
        for step, data_packet in enumerate(pbar):
            ...
            if modal_names == ["Visual", "Audio"]:
                ...
                if args.dataset == 'CREMAD':
                    ...
                    data_mini_packet = (image.float(), spec.unsqueeze(1).float())
                outputs = model(data_mini_packet,sid=sid)
            elif modal_names == ["Image", "Text"]:
...
def main():
    ...
    if args.dataset == 'CREMAD':
        ...
        args.num_classes = 3
        train_dataset = M3AEDataset(args,mode='train', return_raw=True)
        test_dataset = M3AEDataset(args,mode='test', return_raw=True)
    ...
    # test_size = int(0.15 * len(test_dataset))
    ...
    # print("train_dataset size: {}".format(train_size))
    # print("test_dataset size: {}".format(len(test_dataset)))
    # train_dataloader = DataLoader(train_dataset_subset, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True)
    # test_dataloader = DataLoader(test_dataset_subset, batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True)
    ...
    model.to(device)
    if args.model_name == '["Visual","Audio"]' and args.Use_initWeight and args.Use_OLA==False:
    ...
    train_log_path = os.path.join(args.ckpt_path, args.dataset + '_' + args.fusion_method + '_train.csv')
    modal_names = ast.literal_eval(args.model_name)
    modal_header = []
    train_modal_header = []
    for name in modal_names:
        modal_header.extend([f"Val_Acc_{name}", f"Val_F1_{name}",f"{name}_mask_number"])
        train_modal_header.extend([f"Train_Acc_{name}", f"Train_F1_{name}", f"Train_Loss_{name}"])
    with open(log_path, 'a+', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter=",")
        writer.writerow(['Epoch', 'Val_Acc', 'Val_F1', *modal_header])
    with open(train_log_path, 'a+', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter=",")
        writer.writerow(['Epoch', 'Train_Acc_Fusion', 'Train_F1_Fusion', 'Train_Loss_Fusion', *train_modal_header])
    # ==================训练和验证=====================================
    if args.train:
        best_acc = 0.0
        save_path = None
        ...
        for epoch in range(args.current_epoch, args.epochs + 1):
            ...
            with open(train_log_path, 'a+', newline='') as csvfile:
                writer = csv.writer(csvfile, delimiter=",")
                row = [epoch, train_fusion_metrics[0], train_fusion_metrics[1], train_fusion_metrics[2]]
                for name in modal_names:
                    train_acc, train_f1, train_loss = train_modal_metrics.get(name, (0.0, 0.0, 0.0))
                    row.extend([train_acc, train_f1, train_loss])
                writer.writerow(row)
        # with open(f"Results/results-AME-{args.dataset}-{args.fusion_method}.log", "a") as f:
        with open(f"Results/results-AMRe-OLA-{args.fusion_method}.log", "a") as f:
            f.write(
